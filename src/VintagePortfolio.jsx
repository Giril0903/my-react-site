  ...
  const tracks = [
    {
      ...
      cover: "https://drive.google.com/uc?export=view&id=1i_gf-kkIWBywcQh36DngHyDrHbQe0949",
        ...
        <div className="max-w-6xl mx-auto px-4">
          ...
          <div className="mt-6 grid md:grid-cols-2 gap-6 items-stretch">
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="relative rounded-2xl overflow-hidden border border-white/15 bg-white/[0.04] shadow-[0_10px_30px_-20px_rgba(0,0,0,.6)]">
              ...
              />
              {!videoLoaded && (
                <a href={`https://youtu.be/${ytId}`} target="_blank" rel="noreferrer" className="absolute inset-0 grid place-items-center text-white/90">
                  <span className="inline-grid place-items-center w-16 h-16 rounded-full border border-white/30 bg-black/40 backdrop-blur text-xl">▶</span>
                </a>
              )}
            </div>
            </motion.div>
